(function(){
  try{
    ...
    function createBubble(color, config){
      ...
      b.innerHTML='<svg width="28" height="28" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
      return b;
    }
    function createWrap(config){
      ...
      css(bar,{
        ...
        borderBottom:'1px solid #eef2f7',display:'flex',alignItems:'center',justifyContent:'flex-end',
        padding:'0 10px',fontSize:'14px',color:'#111827',zIndex:1
      });
      // Remove title bar content
    ...
    function open(){wrap.style.opacity='0';wrap.style.transform='scale(0.8)';wrap.style.display='block';bubble.style.display='none';setTimeout(function(){wrap.style.transition='opacity 0.2s ease, transform 0.2s ease';wrap.style.opacity='1';wrap.style.transform='scale(1)'},10); try{localStorage.setItem(KEY,'1')}catch(e){} }
    function close(){wrap.style.transition='opacity 0.2s ease, transform 0.2s ease';wrap.style.opacity='0';wrap.style.transform='scale(0.8)';setTimeout(function(){wrap.style.display='none';bubble.style.display='flex'},200); try{localStorage.setItem(KEY,'0')}catch(e){} }
